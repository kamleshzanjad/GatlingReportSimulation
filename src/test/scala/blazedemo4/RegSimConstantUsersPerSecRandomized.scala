...
import io.gatling.core.Predef.{BlackList, WhiteList, csv, feed, scenario}
...
class RegSimConstantUsersPerSecRandomized extends Simulation {
	...
	setUp(
		scnRefactor.inject(
		//atOnceUsers(10))
	  //	rampUsers(20) during (20)  constantUsersPerSec(rate).during(duration).randomized
			constantUsersPerSec(20).during (20).randomized
	)
	)
